...
class Script < Module
  ...
  def load_in_module(file)
    @__script_file = File.expand_path(file)
    _load_in_module
  ...
  def _load_in_module # :nodoc:
    # No extra locals in this scope.
    module_eval("@__script_scope = proc{}\n" + IO.read(@__script_file),
      @__script_file, 0) # start numbering at 0 because of the extra line.
  end
  ...
  module ScriptModuleMethods
    ...
    # Call this method on your script to make local scope, including local vars,
    # accessible via the script object. See scope example. This can be
    # called within the script itself or it can be called on the script by
    # the loading program.
  ...
  module AccessibleScope
    ...
    def local(name)
      unless name.is_a?(Symbol)
        raise ArgumentError, "Variable name must be symbol, not #{name.class}"
      end
